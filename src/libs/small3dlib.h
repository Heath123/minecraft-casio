  ...
  S3L_Index triangleIndex)
{
  ...
  // printf("(%d, %d), (%d, %d), (%d, %d), %04X\n",
  //   point0.x, point0.y,
  //   point1.x, point1.y,
  //   point2.x, point2.y,
  //   colours[triangleIndex]
  // );
  // (239, 132), (286, 132), (194, 108)
  azrp_triangle(
    point0.x /* / S3L_FRACTIONS_PER_UNIT */,
    point0.y /* / S3L_FRACTIONS_PER_UNIT */,
    point1.x /* / S3L_FRACTIONS_PER_UNIT */,
...
static void quicksort(_S3L_TriangleToSort* arr, int16_t low, int16_t high)
{
    if(low >= high) return;

    _S3L_TriangleToSort pivot = arr[(low + high) >> 1];
    ...
    while(1) {
        do i++;
        while(arr[i].sortValue inverse_cmp pivot.sortValue);

        do j--;
        while(arr[j].sortValue cmp pivot.sortValue);

        if(i >= j) break;

        _S3L_TriangleToSort tmp = arr[i];
        arr[i] = arr[j];
        arr[j] = tmp;
    }
...
void S3L_drawScene(S3L_Scene scene)
{
  ...
  for (modelIndex = 0; modelIndex < scene.modelCount; ++modelIndex)
  {
    ...
    // if (scene.models[modelIndex].config.alwaysOnTop)
  ...
  quicksort(S3L_sortArray, 0, S3L_sortArrayLength - 1);
